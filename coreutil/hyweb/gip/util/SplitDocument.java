...
public class SplitDocument {
	...
	private String ori_context; /* 文章內文 */
	private String context; //  "<x:xxx>" + context + "</x:xxx>";
	private int wordsPerPage = 600; /* 一頁幾個字，不含HTML TAG */
	private String baseFileName; /* 存檔的檔名，ex:page1為 baseFileName + "_1.txt" */
	private File baseDir; /* 存檔的目錄 */
	private int cc;
	...
	public void split_page(StringBuffer sb, int ps, int pe) {
		...
		while (idx < len) {
			...
			//System.out.println("----->  cc : " + cc  + " asc : " +(int)c+ " : " + c + " idx : " + idx); 
	...
	public void process_files() {
		...
		try {
			saveToFile(new File(baseDir,baseFileName+".txt"), context,"UTF-8");
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		int idx = 1;
